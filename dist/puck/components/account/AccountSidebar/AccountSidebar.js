...
export const accountSidebarFields = {
...
export const AccountSidebar = {
    ...
    render: ({ showOrders, showProfile, showAddresses, showWishlist, showPaymentMethods, showConcierge, showRewards, showLogout, style, activeView }) => {
        ...
        return (_jsxs("nav", { className: `flex flex-col gap-1 ${luxury ? 'font-light tracking-wide' : ''}`, children: [items.map((item) => (_jsx("a", { href: "#", className: `no-underline px-4 py-2 rounded-md text-sm transition-colors ${item.key === activeView ? 'bg-gray-900 text-white' : 'text-gray-700 hover:bg-gray-100'}`, children: item.label }, item.key))), showLogout && (_jsx("a", { href: "#", className: "no-underline px-4 py-2 rounded-md text-sm text-red-600 hover:bg-red-50 mt-2 border-t border-gray-100 pt-3", children: "Logout" }))] }));
    },
};

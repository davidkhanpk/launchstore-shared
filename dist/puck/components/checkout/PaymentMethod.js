...
import { resolvePaymentMeta } from '../../payment-info';
...
export const PaymentMethod = {
    ...
    render: (raw) => {
        const { layout = 'list', showPaymentIcons, showSecurityBadges, enableSaveCard } = raw;
        ...
        const renderable = methods.map((m) => {
            const meta = resolvePaymentMeta(m.id);
            return {
                id: m.id,
        ...
        return (_jsxs("div", { className: "border border-gray-200 rounded-lg p-6 bg-white", children: [_jsxs("div", { className: "flex items-center gap-3 mb-6 pb-4 border-b border-gray-200", children: [_jsx(Card, {}), _jsx("h2", { className: "text-xl font-semibold text-gray-900", children: "Payment Method" })] }), _jsx("div", { className: "space-y-4 mb-6", children: renderable.map((method) => (_jsx("div", { onClick: () => onSelect(method.id), className: `border-2 rounded-lg p-4 transition-colors cursor-pointer ${method.id === selectedId ? 'border-blue-500 bg-blue-50' : 'border-gray-200 hover:border-blue-500'}`, children: _jsxs("div", { className: "flex items-center gap-3", children: [_jsx("input", { type: "radio", name: "payment", className: "h-4 w-4", checked: method.id === selectedId, onChange: () => onSelect(method.id) }), showPaymentIcons && method.icon && _jsx("span", { className: "flex-shrink-0 w-5 h-5 flex items-center justify-center text-gray-700", children: method.icon }), _jsxs("div", { className: "flex-1", children: [_jsx("h4", { className: "font-medium text-gray-900", children: method.name }), method.description && _jsx("p", { className: "text-sm text-gray-600", children: method.description })] })] }) }, method.id))) }), enableSaveCard && (_jsx("div", { className: "border-t border-gray-200 pt-4 mb-6", children: _jsxs("label", { className: "flex items-center", children: [_jsx("input", { type: "checkbox", className: "mr-2 h-4 w-4" }), _jsx("span", { className: "text-sm text-gray-700", children: "Save card for future purchases" })] }) })), showSecurityBadges && (_jsx("div", { className: "border-t border-gray-200 pt-4 mb-6", children: _jsxs("div", { className: "flex items-center justify-center gap-6 text-sm text-gray-600", children: [_jsxs("div", { className: "flex items-center gap-2", children: [_jsx(Lock, {}), " ", _jsx("span", { children: "SSL Encrypted" })] }), _jsxs("div", { className: "flex items-center gap-2", children: [_jsx(Shield, {}), " ", _jsx("span", { children: "PCI Compliant" })] })] }) })), showContinueButton && (_jsx("div", { className: "mt-6", children: _jsxs("button", { type: "button", onClick: onContinue, disabled: isLoading, className: "w-full bg-black text-white py-3 px-6 rounded-lg hover:bg-gray-800 transition-colors font-medium flex items-center justify-center gap-2 disabled:opacity-60 disabled:cursor-not-allowed", children: [isLoading && _jsx(Spinner, { size: 18 }), isLoading ? loadingText : 'Review Order'] }) }))] }));
    },
};

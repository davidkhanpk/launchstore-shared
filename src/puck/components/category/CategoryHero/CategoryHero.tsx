...
const HEIGHT: Record<CategoryHeroProps['height'], string> = { small: 'h-40', medium: 'h-64', large: 'h-96' };

export const categoryHeroFields: ComponentConfig<CategoryHeroProps>['fields'] = {
  ...
  render: ({ showImage, showTitle, showDescription, height, overlayOpacity, style, backgroundColor, textColor, title, description, image }) => {
    ...
    return (
      <div className={`relative w-full ${HEIGHT[height] || 'h-64'} flex items-center justify-center overflow-hidden`} style={{ backgroundColor: bg }}>
        {showImage && image && <img src={image} alt={title || 'Category'} className="absolute inset-0 w-full h-full object-cover" />}
        {showImage && <div className="absolute inset-0" style={{ backgroundColor: '#000', opacity: overlayOpacity ?? 0.4 }} />}
        <div className="relative text-center px-4" style={{ color: fg }}>
          ...
          {showDescription && (
            <p className="mt-2 text-base opacity-90">{description || 'Browse our collection'}</p>
          )}
        </div>
      </div>
    );

 ...
 */
export const SORT_KEY_TO_MEDUSA: Record<SortKey, string> = {
...
export interface FilterBarProps {
  ...
  // Data (injected by the storefront wrapper — never hardcoded)
  ...
  // Callbacks (wired to URL state by the storefront wrapper)
  onSortChange?: (sort: SortKey) => void;
  onFilterChange?: (group: string, value: string) => void;
  onClearAll?: () => void;
}

// ── PaginationBar Component Props ──────────────────────────────────────────
...
export interface SimpleProductGridProps {
  ...
  // Data (injected by storefront wrapper)
  products?: any[];
  loading?: boolean;
